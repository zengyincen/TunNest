<div align="center">
  ...
  <p>
    ...
    <img src="https://img.shields.io/badge/release-v1.1.0-1D1D1F?style=flat-square" alt="Release v1.1.0">
...
**囤囤不是另一个内容平台，而是一条通往你私人资料库的路。** 它把不同来源整理成统一结构，直接写进你的 Notion。搜索、标签、回顾和长期保存，都重新回到你手里。
...
| 豆瓣 | 书 / 影 / 音状态、评分、短评 | ✅ | ✅ Frodo GET API |
...
- **写进你自己的数据库**：Notion 页面归你管理，可以继续加标签、关联项目或做年度回顾。
...
首次连接时，囤囤可以在指定父页面下创建一个统一资料库。不同来源会尽量映射到一致字段：

| 字段 | 示例 |
|---|---|
| 标题 | 《献给阿尔吉侬的花束》/ 一条微博 / 网页标题 |
| 来源 | 微信读书、豆瓣、微博、网页 |
| 类型 | 书籍、划线、笔记、电影、短评、博文、文章 |
| 原始链接 | 回到对应平台或网页 |
| 作者 / 用户 | 作者、导演或微博用户 |
| 评分 / 状态 | 豆瓣评分、想看 / 看过、在读 / 读过 |
| 正文 | 划线、笔记、短评、博文或网页选区 |
| 收藏时间 | 写入 Notion 的时间 |

你可以在 Notion 中继续搭建「今年读过的书」「爱豆博文档案」「周末片单」或「灵感素材库」，囤囤只负责把原料安静地送到。
