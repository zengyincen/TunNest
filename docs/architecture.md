...
```mermaid
flowchart LR
  B["浏览器扩展"] -->|"按来源路由"| N["用户的七个 Notion 数据库"]
  G["GitHub Actions\n微信读书 / 豆瓣"] -->|"分别写入专属数据库"| N
...
- 网页剪藏、微信读书和微博各保存一个数据库 ID；豆瓣保存用户收藏及三套 Top 250 共四个数据库 ID。七个数据库共用一个 Integration Token，豆瓣四库共用一个父页面。
- 微信读书网页 Cookie 和微博 Cookie 不离开浏览器。
- Worker 处理订阅授权并流式代理公开的豆瓣封面，不接收正文、Cookie、Notion Token 或用户资料库内容；图片只进入边缘缓存，不写入 D1。
